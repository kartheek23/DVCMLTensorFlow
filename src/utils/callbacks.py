...
def create_and_save_checkpoint_callback(callbacks_dir,checkpoint_dir):
    ...
    ckpt_callback_filepath = os.path.join(callbacks_dir,"checkpoints_cb.cb")
...
def get_callbacks(callback_dir_path):
    callback_path = [os.path.join(callback_dir_path,bin_file) for bin_file in os.listdir(callback_dir_path) if bin_file.endswith(".cb")]
